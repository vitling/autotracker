...
function getNoteName({note, fx, vel, drum}: Slot) {
    let name = "";
    if (note === "---") { name = "---"; }
    else if (note === 'cont') { name =  "&nbsp;&nbsp;&nbsp;"; }
    else if (drum) { name = drum; }
    else {
        name = drum ? drum : ['A-', 'A#', 'B-', 'C-', 'C#', 'D-', 'D#', 'E-', 'F-', 'F#', 'G-', 'G#'][(note - A0) % 12] + Math.floor((note - A0) / 12)
    }

    if (fx && fx.pulseWidth) {
        name = name + " w" + Math.floor(fx.pulseWidth * 255).toString(16).toUpperCase();
    }

    if (fx && fx.glide) {
        name = name + " g" + Math.floor(fx.glide * 255).toString(16).toUpperCase();
    }

    if (vel) {
        name = name + " v" + Math.floor(vel * 255).toString(16).toUpperCase();
    }

    return name;
}

function fill<T>(count: number, fn: (number) => T): T[] {
...
const gen = {
    arp: () => {
        ...
        return fill(PatternSize, i => {
            ...
            const chord = triad.map(noteIndex => key + scale[(chordNumber - 1 + noteIndex) % scale.length]);
            return {
    ...
    melody1: () => {
        ...
        for (let i = 0; i < PatternSize; i++) {
            ...
            // const progIndex = Math.floor(i/4);
            // const chordNumber = progression[progIndex];
            // const chord = triad.map(noteIndex => key + scale[(chordNumber - 1 + noteIndex - 1) % scale.length]);
        }
        return pattern;
    },
    empty: () => fill(PatternSize, i => ({note: '---'} as Slot)),
...
function replacePatterns(newPats: Pattern[]) {
    ...
    function add(pattern: Pattern, index: number) {
        ...
        pDisplay.innerHTML =
            ...
            pattern.map((x, i) => "<div class='note' data-index='" + i + "'>" + getNoteName(x) + "</div>").join("");
...
function DrumSynth(ctx: AudioContext) {
    ...
    function play(slot: Slot) {
        ...
        if (slot.note === "---") {
            ...
            //gain.gain.setTargetAtTime(0.0,ctx.currentTime, 0.08);
        }
    }
    return {
        play,
...
function start() {
    ...
    function frame(f: number) {
        ...
        if (f % 1024 === 0) {
            ...
            handle.timer = window.setInterval(doFrame, (60000/bpm)/4);
        }
        if (f % 512 === 0) {
            modulate();
        }
        if (f % 256 === 0) {
            progression = choose(progressions);
        }
        if (f % 128 === 0) {
            replacePatterns([
        ...
        hl.highlight(positionInPattern);
